document.addEventListener("DOMContentLoaded", function () {
  ...
  const houseSizeFromUrl = urlParams.get("houseSize");
  const tileTypeFromUrl = urlParams.get("tileType");

  if (houseSizeFromUrl && tileTypeFromUrl) {
    const priceRanges = {
      small: {
        clay: "$5,000 - $7,000",
    ...
    priceEstimate.textContent = `Estimated Price Range: ${
      priceRanges[houseSizeFromUrl]?.[tileTypeFromUrl] || "Contact us for a custom quote"
    }`;
  } else {
